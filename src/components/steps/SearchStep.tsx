...
import { Input } from "@/components/ui/input";
  ...
  const copyLeadsFromLists = async (sourceListIds: string[], targetCampaignId: string, targetListId: string) => {
    ...
    if (projectId) {
      const { data: projectCampaigns } = await supabase.from("campaigns").select("id").eq("project_id", projectId);
      const campaignIds = (projectCampaigns || []).map((c) => c.id);
      if (campaignIds.length > 0) {
        const { data } = await supabase.from("leads").select("email, linkedin_url").in("campaign_id", campaignIds);
